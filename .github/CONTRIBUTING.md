...
Maintainers create release tags manually. Tags must use `vMAJOR.MINOR.PATCH`
format, must match the source version, and must not be moved, force-pushed,
deleted, or reused after publication. If a release is wrong, publish a new patch
tag instead of rewriting the old one.

Release assets are built by GitHub Actions from the tag and uploaded to the
GitHub Release. They must not be committed to git. `CHANGELOG.md` remains the
human-maintained summary; release pages use commit-driven notes generated by CI.

维护者手动创建 release tag。Tag 必须使用 `vMAJOR.MINOR.PATCH` 格式，必须与源码版本一致，
发布后不得移动、强推、删除或复用。发布错误时创建新的 patch tag，不重写旧 tag。

Release assets 由 GitHub Actions 从 tag 构建并上传到 GitHub Release，不提交到 git。
`CHANGELOG.md` 保留为人工维护概要；release 页面使用 CI 生成的 commit-driven notes。
