...
# ═══════════════════════════════════════════════════════════════════════════════
# Pure-Python source TC resolver (replaces ffprobe dependency)
# ═══════════════════════════════════════════════════════════════════════════════

_SourceTCCache: dict[str, _SourceTCInfo] = {}


def _parse_source_tc_from_file(filepath: str) -> _SourceTCInfo:
    """Pure-Python source timecode extraction from MP4/MOV files.

    Priority order:
      1. tmcd track sample (DJI MP4) — raw frames since midnight → TC
      2. creation_time atom (A001 MOV) — ISO 8601 UTC → local TC
      3. Returns unresolved (caller falls back to MediaInPoint or 00:00:00:00)

    No external dependencies.  Reads only binary atom headers.
    """
    info = _SourceTCInfo()
    try:
    ...
    # ── Find moov box (may be at start or end) ──
    moov = _find_moov_box(data)
    if moov is None:
        return info

    # ── Extract r_frame_rate from the first video track ──
    _read_video_frame_rate(moov, info)
    info.is_ntsc = _is_ntsc_fps(info.media_fps)

    # ── Path 1: tmcd track timecode ──
    if _read_tmcd_timecode(data, moov, info):
        return info

    # ── Path 2: creation_time (MOV/MP4) ──
    if _read_creation_time_to_tc(moov, info):
        return info

    return info


def _find_moov_box(data: bytes) -> bytes | None:
    """Locate the moov box in MP4/MOV data."""
    off = 0
    # Scan first 64KB for moov (non-streaming files)
    header_end = min(65536, len(data))
    while off + 8 < header_end:
        sz = _be_u32(data, off)
...
def _read_video_frame_rate(moov: bytes, info: _SourceTCInfo) -> None:
    """Extract r_frame_rate from the first video track's stsd."""
    # Walk moov children to find first vide trak
    off = 8 if moov[4:8] == b"moov" else 0
    while off + 8 < len(moov):
        ...
        if nm == b"trak":
            ...
            # Find handler type
            htype = _find_handler_type(trak)
            if htype == b"vide":
                _read_fps_from_stsd(trak, info)
                return
        off += sz


def _find_handler_type(trak: bytes) -> bytes | None:
    """Find the QuickTime handler type (vide/soun/tmcd) in a trak."""
    mdia = _find_child_box(trak, b"mdia")
    if not mdia:
        return None
    hdlr_body = _find_child_box(mdia, b"hdlr")
    # hdlr: [size(4)][hdlr(4)][ver(1)+flg(3)][comp_type(4)][subtype(4)]...
    if hdlr_body and len(hdlr_body) >= 20:
        return hdlr_body[16:20]
    return None


def _read_fps_from_stsd(trak: bytes, info: _SourceTCInfo) -> None:
    """Read frame rate from video track's stsd sample entry."""
    mdia = _find_child_box(trak, b"mdia")
    if not mdia:
        return
    ...
    if not stbl:
        return
    stsd_body = _find_child_box(stbl, b"stsd")
    # stsd: [size(4)][stsd(4)][ver+flg(4)][entry_count(4)][entries...]
    if not stsd_body or len(stsd_body) < 20:
        return
    e_sz = _be_u32(stsd_body, 16)
    if e_sz < 8 or 16 + e_sz > len(stsd_body):
        return
    ebody = stsd_body[24:16+e_sz]
    # Different codecs have different field layouts.  Try common patterns.
    # 'avc1'/'hvc1': skip to width/height
    if len(ebody) >= 36:
        w = struct.unpack(">H", ebody[32:34])[0]
        h = struct.unpack(">H", ebody[34:36])[0]
        if 0 < w < 20000 and 0 < h < 20000:
            info.source_w = w
            info.source_h = h
    # Fallback fps from mdhd
    mdhd_body = _find_child_box(mdia, b"mdhd")
    # mdhd: [size(4)][mdhd(4)][ver(1)+flg(3)][creation(4)][mod(4)][timescale(4)]
    if mdhd_body and len(mdhd_body) >= 24:
        ver = mdhd_body[8]
        ts_off = 20 if ver == 0 else 28
        if len(mdhd_body) >= ts_off + 4:
            timescale = _be_u32(mdhd_body, ts_off)
            if timescale >= 1000:
                info.media_fps = float(timescale)


def _read_tmcd_timecode(data: bytes, moov: bytes, info: _SourceTCInfo) -> bool:
    """Read timecode from tmcd track's sample data. Returns True on success."""
    # Find tmcd trak
    tc_trak = _find_trak_by_handler(moov, b"tmcd")
    if not tc_trak:
        return False

    # Get stsd → timeScale + frameDuration
    stbl = _walk_to_stbl(tc_trak)
    if not stbl:
        return False
    stsd_body = _find_child_box(stbl, b"stsd")
    if not stsd_body or len(stsd_body) < 16:
        return False

    tscale = 0
    fdur = 0
    # stsd: [size(4)][stsd(4)][ver(1)+flg(3)][entry_count(4)][entries...]
    entry_count = _be_u32(stsd_body, 12)
    pos = 16
    for _ in range(entry_count):
        if pos + 8 > len(stsd_body):
            break
        esz = _be_u32(stsd_body, pos)
        if esz < 8 or pos + esz > len(stsd_body):
            break
        efmt = stsd_body[pos+4:pos+8]
        ebody = stsd_body[pos+8:pos+esz]

        if efmt == b"tmcd" and len(ebody) >= 24:
            tscale = _be_u32(ebody, 16)
            fdur = _be_u32(ebody, 20)
        pos += esz

    if tscale <= 0 or fdur <= 0:
        return False

    # Read sample from stco
    stco_body = _find_child_box(stbl, b"stco")
    # stco: [size(4)][stco(4)][ver(1)+flg(3)][entry_count(4)][offsets...]
    if not stco_body or len(stco_body) < 20:
        return False
    nchunks = _be_u32(stco_body, 12)
    if nchunks == 0:
        return False
    chunk_off = _be_u32(stco_body, 16)

    if chunk_off + 4 > len(data):
        return False
    counter = struct.unpack(">I", data[chunk_off:chunk_off+4])[0]

    # Convert counter → TC string
    seconds = (counter * fdur) / tscale
    display_fps = int(round(tscale / fdur))
    if display_fps <= 0:
        display_fps = 60
    ...
    ss = int(seconds % 60)
    ff = int(round((seconds - int(seconds)) * display_fps))
    if ff >= display_fps:
        ff = display_fps - 1

    info.timecode_string = f"{hh:02d}:{mm:02d}:{ss:02d}:{ff:02d}"
    info.media_fps = round(tscale / fdur, 3)
    info.is_ntsc = _is_ntsc_fps(info.media_fps)
...
def _read_creation_time_to_tc(moov: bytes, info: _SourceTCInfo) -> bool:
    """Extract creation_time from moov.udta and convert to local TC.

    For A001 MOV: moov.udta.meta.ilst contains '©day' atom with ISO 8601
    UTC creation time.  Convert UTC → local timezone, format as hh:mm:ss:00.
    """
    udta = _find_child_box(moov, b"udta")
    if not udta:
        return _read_mvhd_creation(moov, info)

    meta = _find_child_box(udta, b"meta")
    if not meta:
        return _read_mvhd_creation(moov, info)

    # Walk ilst looking for '©day' atom
    ilst = _find_child_box(meta, b"ilst")
    if not ilst:
        return _read_mvhd_creation(moov, info)

    # ilst body: items as [size(4)][fourcc(4)][data_box...]
    ilst_body = ilst[8:]  # skip ilst header
    return _find_cday_in_ilst(ilst_body, info) or _read_mvhd_creation(moov, info)


def _find_cday_in_ilst(ilst_body: bytes, info: _SourceTCInfo) -> bool:
    """Search ilst items for '©day' (0xA9 0x64 0x61 0x79)."""
    off = 0
    while off + 12 < len(ilst_body):
        sz = _be_u32(ilst_body, off)
        if sz < 12 or off + sz > len(ilst_body):
            break
        fourcc = ilst_body[off+4:off+8]
        # ©day = 0xA9 0x64 0x61 0x79
        _CDAY = bytes([0xA9, 0x64, 0x61, 0x79])
        if fourcc == _CDAY:
            data_box = ilst_body[off+8:off+sz]
            if len(data_box) >= 8:
                payload = data_box[8:]
                try:
                    ...
                    break
                if ct:
                    _iso8601_to_tc(ct, info)
                    return info.resolved
        off += sz
    return False


def _read_mvhd_creation(moov: bytes, info: _SourceTCInfo) -> bool:
    """Fallback: read mvhd creation_time (Mac epoch, seconds since 1904)."""
    mvhd_body = _find_child_box(moov, b"mvhd")
    # mvhd: [size(4)][mvhd(4)][ver+flg(4)][creation(4)]...
    if not mvhd_body or len(mvhd_body) < 16:
        return False
    ver = mvhd_body[8]
    ct_off = 12 if ver == 0 else 20
    if len(mvhd_body) < ct_off + 4:
        return False
    mac_epoch = struct.unpack(">I", mvhd_body[ct_off:ct_off+4])[0]
    if mac_epoch == 0:
        return False
    # Mac epoch 1904-01-01 → Unix epoch 1970-01-01: subtract 2082844800
    unix_ts = mac_epoch - 2082844800
    if unix_ts <= 0:
        return False
    dt = datetime.fromtimestamp(unix_ts, tz=timezone.utc).astimezone()
    _format_tc_from_dt(dt, info)
    return info.resolved


def _iso8601_to_tc(ct: str, info: _SourceTCInfo) -> None:
    """Parse ISO 8601 creation_time → local timezone TC."""
    try:
...
def _format_tc_from_dt(dt: datetime, info: _SourceTCInfo) -> None:
    """Format a datetime as hh:mm:ss:00 timecode."""
...
def _find_trak_by_handler(moov: bytes, handler_type: bytes) -> bytes | None:
    """Find the first trak box with the given handler type."""
    off = 8 if moov[4:8] == b"moov" else 0
    while off + 8 < len(moov):
        sz = _be_u32(moov, off)
        if sz < 8 or off + sz > len(moov):
        ...
        if moov[off+4:off+8] == b"trak":
            if _find_handler_type(moov[off:off+sz]) == handler_type:
                return moov[off:off+sz]
        off += sz
    return None


def _walk_to_stbl(trak: bytes) -> bytes | None:
    ...
    if not minf:
        return None
    return _find_child_box(minf, b"stbl")
...
def _find_child_box(
    ...
    """Find and return the first child box (including its 8-byte header)."""
    parent_body = parent[8:]  # skip parent's own header
    off = 0
    while off + 8 < len(parent_body):
        sz = _be_u32(parent_body, off)
        if sz < 8 or off + sz > len(parent_body):
            break
        if parent_body[off+4:off+8] == box_type:
            return parent_body[off:off+sz]  # full child atom
        off += sz
    return None


def _be_u32(buf: bytes, offset: int) -> int:
                    ...
                    if media_path:
                        ...
                        if local.exists():
                            ...
                            if local_str in _SourceTCCache:
                                ...
                                ff_tc = _parse_source_tc_from_file(local_str)
