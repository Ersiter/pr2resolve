...
def test_ffprobe_tc_priority_over_mediainpoint():
    ...
    with patch("pr2_engine._parse_source_tc_from_file", side_effect=fake_ffprobe), \
...
def test_mediainpoint_fallback_without_ffprobe():
    ...
    with patch("pr2_engine._parse_source_tc_from_file", side_effect=fake_ffprobe_unavailable):
        fcp = _prproj_parse_sequence(root, primary.get("ObjectUID"), test_proj)
...
def test_mvi_zero_tc_regression():
    ...
    with patch("pr2_engine._parse_source_tc_from_file", side_effect=fake_ffprobe_none), \
         patch("pathlib.Path.exists", return_value=True), \
         patch("pr2_engine.Path.exists", return_value=True):
        fcp = _prproj_parse_sequence(root, primary.get("ObjectUID"), test_proj)
...
def test_creation_time_to_local_tc():
    """PR4: creation_time (UTC) must be converted to local timezone TC.

    Tests _iso8601_to_tc directly with a UTC timestamp.  Verifies that
    the ISO 8601 string is parsed and converted to local time TC.
    """
    from datetime import datetime, timezone, timedelta
    from pr2_engine import _SourceTCInfo, _iso8601_to_tc

    info = _SourceTCInfo()
    info.media_fps = 30.0

    # 2026-05-30T05:03:40Z → UTC+8 → 13:03:40:00
    _iso8601_to_tc("2026-05-30T05:03:40.000000Z", info)
    # _iso8601_to_tc uses astimezone(), result depends on machine timezone.
    # On UTC+8: TC should be 13:03:40:00
    # On any timezone: at minimum, it should NOT be the raw UTC hour (05)
    assert info.resolved, "creation_time should resolve to TC"
    assert not info.timecode_string.startswith("05:"), (
        f"creation_time must be converted to local time, not raw UTC: {info.timecode_string}"
    )
    # In UTC+8, expected: 13:03:40:00
    if timezone(timedelta(hours=8)).utcoffset(None) == timezone(timedelta(hours=8)).utcoffset(None):
        pass  # timezone check is conceptual — skip exact match in CI
...
def test_dji_timecode_priority():
    ...
    with patch("pr2_engine._parse_source_tc_from_file", side_effect=fake_ffprobe_dji), \
         patch("pathlib.Path.exists", return_value=True):
        fcp = _prproj_parse_sequence(root, primary.get("ObjectUID"), test_proj)
...
def test_no_metadata_fallback():
    ...
    with patch("pr2_engine._parse_source_tc_from_file", side_effect=fake_ffprobe_no_metadata), \
         patch("pathlib.Path.exists", return_value=True), \
         patch("pr2_engine.Path.exists", return_value=True):
        fcp = _prproj_parse_sequence(root, primary.get("ObjectUID"), test_proj)
...
def test_dropframe_ndf_to_df_conversion():
    """PR3: raw NDF frame count must produce correct DF label for NTSC rates.

    At 59.94fps (display 60), 1 minute = 3600 raw frames.
    Minute 0 (non-10th) drops 2 frames, so labels 3598 frames.
    Raw frame 3600 → DF label 00:01:00;02 (not 00:01:00;00).
    """
    from pr2_engine import _prproj_frames_to_timecode_string

    # 29.97fps: raw NDF frame 1800 → DF label 00;01;00;02
    tc = _prproj_frames_to_timecode_string(1800, 29.97, True)
    assert tc == "00;01;00;02", (
        f"DF at 29.97: raw 1800 should be 00;01;00;02, got '{tc}'"
    )

    # 59.94fps: DJI_0208 MIP NDF frames → DF label (all-; separator)
    ndf_dji = 16 * 3600 * 60 + 59 * 60 * 60 + 52 * 60 + 40
    tc2 = _prproj_frames_to_timecode_string(ndf_dji, 59.94, True)
    assert tc2 == "17;00;23;16", (
        f"DF at 59.94: raw {ndf_dji} should be 17;00;23;16, got '{tc2}'"
    )

    # 30.0 NDF: no change (is_ntsc=False path untouched)
    tc3 = _prproj_frames_to_timecode_string(1800, 30.0, False)
    assert tc3 == "00:01:00:00", (
        f"NDF regression: 30fps frame 1800 should be 00:01:00:00, got '{tc3}'"
    )

    # 23.976: NDF with ; separator (NTSC convention, not DF)
    tc4 = _prproj_frames_to_timecode_string(24, 23.976, True)
    assert tc4 == "00;00;01;00", (
        f"23.976 NDF: frame 24 should be 00;00;01;00, got '{tc4}'"
    )

    # 0 frame → 00:00:00:00 regardless
    tc5 = _prproj_frames_to_timecode_string(0, 59.94, True)
    assert tc5 == "00:00:00:00", f"Frame 0 should be 00:00:00:00, got '{tc5}'"


def test_dji_mp4_timecode_from_tmcd_sample():
    """PR4: Pure Python reads DJI MP4 tmcd sample → Pool Start TC match.

    DJI_0208 MP4 has a tmcd track (track 5) with sample[0]=3,671,560
    (raw NDF frames since midnight at 60fps).  Pool Start TC = 17:00:53:52.
    """
    from pr2_engine import _parse_source_tc_from_file

    djipath = r"E:\HW\2026.5.29 荷花\Video\DJI_20260528184922_0208_D.MP4"
    if not __import__("os").path.exists(djipath):
        return  # skip if media not available

    tc_info = _parse_source_tc_from_file(djipath)
    assert tc_info.resolved, "DJI MP4 should resolve TC from tmcd sample"
    assert tc_info.timecode_string == "17:00:53:52", (
        f"Expected Pool TC '17:00:53:52', got '{tc_info.timecode_string}'"
    )
    assert tc_info.media_fps == 59.94, (
        f"Expected media_fps=59.94, got {tc_info.media_fps}"
    )
    assert tc_info.is_ntsc is True
